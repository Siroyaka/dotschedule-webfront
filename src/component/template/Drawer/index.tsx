  ...
  return(
    <React.Fragment>
      ...
      <aside className={clsx(className, 'transform', 'duration-300', 'ease-in-out', 'bg-red-300', 'h-full', 'fixed', 'z-50', 'top-0', 'left-0', 'transition-all', 'flex', 'flex-col', 'w-64', `${open ? 'translate-x-0' : '-translate-x-full'}`)}>
        <div id='drawerHeader' className={clsx('h-12', 'bg-orange-300', 'w-full', 'flex', 'flex-row', 'items-center', 'py-1', 'px-4')}>
          <button aria-label='close-left-drawer' className={clsx('py-2 px-2 focus:outline-none transition duration-200 rounded-full', 'hover:bg-orange-200', 'active:bg-orange-100')} style={{transition: 'all .15 ease'}} onClick={onClose}>
            <CloseSvg />
          </button>
        </div>
        {children}
